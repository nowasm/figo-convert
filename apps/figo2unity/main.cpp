...
struct Converter {
    ...
    Baked bake(const Node& n, int scale, bool flatten = false, bool tryNine = false, bool nineOnly = false) {
        ...
        // coords back, so off-frame content keeps its sprites (the root
        // RectMask2D hides it exactly like Figma does).
    ...
    int64_t emit(Node& n, int64_t parentRt, const std::string& name, float pax, float pay,
        ...
        } else if ((isRoot && !inComponent) || n.clipsContent) {
            // Clipping frame (and the prefab root, like figo2godot's forced
            // clip_contents): RectMask2D so glow/shadow bakes and off-frame
            // children don't paint outside the box.
            int64_t maskId = newId();
            emitRectMask2D(maskId, goId);
            comps.push_back(maskId);
        }
